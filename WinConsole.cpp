...
namespace wc {
	...
	short toLinearInd(short x, short y, int width) {
		return y * width + x;
	}
		...
		for(short x = 0; x < MAT_SIZE; x++) {
			for(short y = 0; y < MAT_SIZE; y++) {
				res.dat[x][y] = dat[x][0] * other.dat[0][y] +
			  			  		dat[x][1] * other.dat[1][y] +
			  			  		dat[x][2] * other.dat[2][y];
			}
		}
		return res;
	}
	...
	Vec3 VecTimesMat(Vec3 vec, Mat3 mat) {
		return Vec3(
			(vec.x * mat.dat[0][0]) + (vec.y * mat.dat[0][1]) + (vec.z * mat.dat[0][2]),
			(vec.x * mat.dat[1][0]) + (vec.y * mat.dat[1][1]) + (vec.z * mat.dat[1][2]),
			(vec.x * mat.dat[2][0]) + (vec.y * mat.dat[2][1]) + (vec.z * mat.dat[2][2])
		);
	}
	
	Vec3 VecTimesMat(Mat3 mat, Vec3 vec) {
		return Vec3(
			(vec.x * mat.dat[0][0]) + (vec.y * mat.dat[1][0]) + (vec.z * mat.dat[2][0]),
			(vec.x * mat.dat[0][1]) + (vec.y * mat.dat[1][1]) + (vec.z * mat.dat[2][1]),
			(vec.x * mat.dat[0][2]) + (vec.y * mat.dat[1][2]) + (vec.z * mat.dat[2][2])
		);
	}
	
	void printVec(Vec3 o) {
		printf("%.2f %.2f %.2f", o.x, o.y, o.z);
	}
	...
	bool WinConsole::tick(float elapsed) { 
		return false; 
	}
	
	bool WinConsole::render() { 
		return false; 
	}
	...
	void WinConsole::setWidth(short width) {
		this->width = width;
	}
	
	void WinConsole::setHeight(short height) {
		this->height = height;		
	}
	
	void WinConsole::setFontWidth(short fontw) {
		this->fontw = fontw;
	}
	
	void WinConsole::setFontHeight(short fonth) {
		this->fonth = fonth;		
	}
			
	char *WinConsole::getBuffer() {
		return buffer;
	}
	
	unsigned short WinConsole::getWidth() {
		return width;
	}
	
	unsigned short WinConsole::getHeight() {
		return height;
	}
	
	unsigned short WinConsole::getFontWidth() {
		return fontw;
	}
	
	unsigned short WinConsole::getFontHeight() {
		return fonth;
	}
	
	float WinConsole::getElapsed() {
		return elapsed;
	}

	Mat3 &WinConsole::getTranslation() {
		return translation;
	}
	
	Mat3 &WinConsole::getRotation() {
		return rotation;
	}
	
	Mat3 &WinConsole::getScalar() {
		return scalar;
	}
	
	Mat3 &WinConsole::getTransform() {
		return transform;
	}
	...
	bool WinConsole::getKey(char k) {
		return keys[(unsigned short)(k)];
	}
}
